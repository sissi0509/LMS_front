...
export default function QuizDetail({courseId, quizId}: {courseId: string; quizId: string}) {
    ...
    const getTime = (date: string) => {
        ...
        const dateS = dateObj.toLocaleDateString('en-US', {timeZone: "UTC", month: 'short', day: '2-digit'});
        const t = dateObj.toLocaleTimeString('en-US', {timeZone: "UTC", hour: '2-digit', minute: '2-digit', hour12: true}).substring(0, 5);
        const z = dateObj.toLocaleTimeString('en-US', {timeZone: "UTC", hour: '2-digit', minute: '2-digit', hour12: true}).substring(6, 8).toLowerCase();
  ...
  return (
    <div>
        ...
        <Form.Control as="textarea" rows={3} plaintext 
            className="border rounded no-click p-3 mb-3" defaultValue={quiz.description} readOnly /> 
        ...
        <Table className="border-secondary">
            ...
            <tbody>
                <tr><td>{getTime(quiz.dueDate)}</td><td>{getTime(quiz.availableFrom)}</td><td>{getTime(quiz.availableUntil)}</td></tr>
            </tbody>
        </Table>
